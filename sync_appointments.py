...
class AppointmentSync:
    ...
    async def _sync_appointments(self, clinic_id: str, appointments: List[Dict], from_date: str = None, to_date: str = None) -> Dict[str, int]:
        ...
        for appointment in appointments:
            try:
                ...
                practitioner_id = self._extract_id_from_link(appointment.get('practitioner', {}).get('links', {}).get('self'))
                patient_id = self._extract_id_from_link(appointment.get('patient', {}).get('links', {}).get('self'))
                business_id = self._extract_id_from_link(appointment.get('business', {}).get('links', {}).get('self'))
                appointment_type_id = self._extract_id_from_link(appointment.get('appointment_type', {}).get('links', {}).get('self'))
                ...
                end_dt = datetime.fromisoformat(ends_at.replace('Z', '+00:00')) if ends_at else start_dt + timedelta(minutes=30)
                # Determine status
                status = 'booked'
