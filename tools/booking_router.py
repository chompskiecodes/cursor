...
from models import (
    ActionType, BookingFor, TranscriptMessage,
    BaseRequest, BookingRequest, AvailabilityRequest,
    CancelRequest, ClinicData, RescheduleRequest,
    BookingResponse,
    PractitionerData,
    ServiceData,
    LocationData,
...
from database import (
    get_clinic_by_dialed_number, find_patient_by_phone,
    get_practitioner_services, match_practitioner,
    match_service, match_business, log_voice_booking,
    invalidate_practitioner_availability, save_appointment_to_db,
    find_appointment_by_details, update_appointment_status,
    get_location_by_name
)
from cliniko import ClinikoAPI
from utils import (
    normalize_phone, mask_phone, normalize_for_matching,
    fuzzy_match, extract_from_transcript,
...
from error_handlers import (
    ...
    handle_no_availability, handle_time_not_available,
    handle_practitioner_location_mismatch, handle_booking_error,
    handle_cancellation_error, create_error_response,
    check_practitioner_location_compatibility, validate_booking_request,
    handle_appointment_creation_api_error
)
from tools.timezone_utils import (
    ensure_utc, parse_cliniko_time, combine_date_time_local,
    format_for_display, format_date_for_display, get_clinic_timezone
)
from .timezone_utils import convert_utc_to_local, format_time_for_voice
...
async def handle_appointment(
    ...
    booking_flow_logger.debug(f"=== BOOKING REQUEST START ===")
...
async def appointment_handler_direct(
    ...
    logger.info(f"=== APPOINTMENT HANDLER DIRECT START ===")
    ...
    try:
        ...
        response = BookingResponse(
            success=True,
            sessionId=session_id,
            message=f"Perfect! I've successfully booked your {service['service_name']} appointment with {practitioner['full_name']} for {display_date} at {display_time} at our {business['business_name']}.",
    ...
    try:
        ...
        if not patient:
            ...
            if len(patient_phone) != 10:
                return create_error_response(
                    error_code="invalid_phone_number",
                    message="Cannot create patient with invalid phone number. Please provide a valid 10-digit Australian phone number.",
                    session_id=request.sessionId
                )
        ...
        if hasattr(request, 'business_id') and request.business_id:
            logger.info(f"=== PRE-RESOLVED LOCATION DEBUG ===")
            ...
            logger.info(f"=== END PRE-RESOLVED LOCATION DEBUG ===")
        ...
        # Otherwise, try to match by location text (EXISTING CODE)
            ...
            logger.info(f"No business_id or location text provided - will use default business")
        ...
        if location_check:
            return create_error_response(
                error_code=location_check['error'],
                message=f"{location_check['practitioner_name']} doesn't work at {location_check['requested_location']}. "
                       f"They are available at: {', '.join(location_check['actual_locations'])}. "
                       f"Would you like to book at one of those locations instead?",
                session_id=request.sessionId
            )
        ...
        logger.info(f"Getting available times for:")
        ...
        if not available_times:
            ...
            practitioner_name = practitioner['full_name'] if isinstance(practitioner, dict) and 'full_name' in practitioner else str(practitioner)
        ...
        if not exact_slot:
            ...
            logger.warning(f"Available slots (first 5):")
        ...
        if not exact_slot:
            ...
            if available_times_local:
                return {
                    ...
                    "message": f"I'm sorry, {display_time} is not available on {display_date}. "
        ...
        return {
            "success": True,
            "message": f"Perfect! I've successfully booked your {service['service_name']} appointment with {practitioner['full_name']} for {display_date} at {display_time}.",
            "appointmentDetails": {
...
async def handle_reschedule(request: RescheduleRequest) -> Dict[str, Any]:
    ...
    try:
        ...
        if not request.appointmentId:
            ...
            if not found_appointment:
                return create_error_response(
                    ...
                    message="I couldn't find your appointment. Could you provide more details like the practitioner's name or the current appointment time?",
                    session_id=request.sessionId
                )
        ...
        current_practitioner_id = str(current_appointment.get('practitioner', {}).get('links', {}).get('self', '').split('/')[-1])
        current_appointment_type_id = str(current_appointment.get('appointment_type', {}).get('links', {}).get('self', '').split('/')[-1])
        current_business_id = str(current_appointment.get('business', {}).get('links', {}).get('self', '').split('/')[-1])
        current_patient_id = str(current_appointment.get('patient', {}).get('links', {}).get('self', '').split('/')[-1])
        ...
        try:
            ...
            try:
                ...
                if not cancel_success:
                    logger.warning(f"Failed to cancel old appointment {request.appointmentId}, but new appointment was created")
            except httpx.HTTPStatusError as e:
                # Check if it's a "slot taken" error
                error_text = str(e.response.text).lower()
                if e.response.status_code == 422 and "already booked" in error_text:
                    ...
                    return create_error_response(
                        ...
                        message="I'm sorry, that time slot was just booked by someone else. Please choose another time.",
                        session_id=request.sessionId
                    )
            ...
            message = f"Perfect! I've successfully rescheduled your appointment"
            ...
            return create_error_response(
                ...
                message="I'm sorry, I encountered an error while rescheduling your appointment. Please try again or contact the clinic directly.",
                session_id=request.sessionId
            )
